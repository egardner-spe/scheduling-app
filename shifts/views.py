...
def time_off_list(request):
    time_off_requests = TimeOffRequest.objects.filter(user=request.user)
...
def request_pickup_shift(request, shift_id):
    shift = get_object_or_404(
        Shift, id=shift_id, is_dropped=True, user=None
    )
    if ShiftPickupRequest.objects.filter(
            shift=shift, requested_by=request.user
    ).exists():
        return redirect('dashboard')

    if request.method == 'POST':
        ShiftPickupRequest.objects.create(
            shift=shift, requested_by=request.user
        )
        return redirect('dashboard')

    return render(request, 'shifts/request_pickup_form.html', {
        'shift': shift
    })
...
# Availability formset (7 weekdays)
...
def view_all_availabilities(request):
    all_avail = Availability.objects.all().order_by('user')
